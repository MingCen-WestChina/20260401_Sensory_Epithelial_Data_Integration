...
lateral_celltype_levels <- c(
  "OV epithelial cells", "Prosensory domain", "Lateral domain",
  "L.PsC", "OHC", "iOHC", "HeC"
)
...
marker_genes_from_xlsx <- list(
  ...
  L.PsD = c("Bmp4", "Rorb", "Camta1", "Lockd", "Fgfr3"),
  L.PsC = c("Fgfr3", "Prox1", "Bmp2", "Tgfbr1", "Ngfr", "Nrcam", "Camta1", "Lockd", "Igfbp3", "Fam159b", "S100a1", "Socs2", "Tectb"),
  OHC = c("Hes6", "Pcp4", "Rprm", "Lhfpl5", "Ccer2", "Pou4f3", "Selenom"),
  iOHC = c("Hes6", "Ccer2", "Pcp4", "Pou4f3", "Selenom"),
  HeC = c("Kazald1", "Pmch", "Fabp7", "Fam159b", "Gata2")
)
...
lateral_marker_genes <- unique(unlist(c(marker_genes_from_xlsx, marker_genes_literature), use.names = FALSE))
...
select_fate_de_genes <- function(expr_mat, meta) {
  ...
  fate_group <- ifelse(cell_type %in% c("OHC", "iOHC"), "OHC",
    ifelse(cell_type %in% c("L.PsC", "HeC"), "supporting", "trunk")
  )
  fate_group <- factor(fate_group, levels = c("trunk", "OHC", "supporting"))
  ...
  unique(c(intersect(lateral_marker_genes, rownames(expr_mat)), selected_genes))
}


#6. ===========Build and order lateral Monocle2 object
...
run_monocle_subset <- function(h5_data) {
  ...
  keep_cells <- rownames(meta)[
    meta$trajectory_celltype %in% lateral_celltype_levels &
  ...
  present_celltype_levels <- lateral_celltype_levels[lateral_celltype_levels %in% unique(meta$trajectory_celltype)]
...
make_branch_labels <- function(cell_df) {
  ...
  trunk_df <- get_group(c("OV", "PsD", "L.PsD"))
  ohc_df <- get_group(c("OHC", "iOHC"))
  support_df <- get_group(c("L.PsC", "HeC"))

  data.frame(
    label = c("OV/PsD/L.PsD", "OHC/iOHC", "L.PsC/HeC"),
    x = c(
      as.numeric(stats::quantile(trunk_df$Component_1, 0.08, na.rm = TRUE)) + 0.35,
      as.numeric(stats::quantile(ohc_df$Component_1, 0.88, na.rm = TRUE)) - 0.20,
      as.numeric(stats::quantile(support_df$Component_1, 0.90, na.rm = TRUE)) - 0.20
    ),
    y = c(
      stats::median(trunk_df$Component_2, na.rm = TRUE) + 0.48,
      as.numeric(stats::quantile(ohc_df$Component_2, 0.18, na.rm = TRUE)) - 0.28,
...
#8. ===========Run and save final lateral trajectory
lateral_data <- load_h5ad_data(input_h5ad_lateral, celltype_col = "trajectory_celltype")
lateral_result <- run_monocle_subset(lateral_data)

p_celltype <- plot_trajectory_panel(
  lateral_result$cds,
  color_by = "celltype_plot",
  title_text = "Lateral trajectory by cell type",
...
p_timepoint <- plot_trajectory_panel(
  ...
  title_text = "Lateral trajectory by timepoint",
...
p_pseudotime <- plot_trajectory_panel(
  ...
  title_text = "Lateral pseudotime"
)

panel_lateral <- gridExtra::arrangeGrob(
  p_celltype,
  p_timepoint,
  p_pseudotime,
  ncol = 3
...
saved_files <- save_plot_pair(
  panel_lateral,
  file.path(output_dir, "Embryonic_E9_E16_panel_a_lateral_real_fatede_trajectory"),
  width = 12.0,
  height = 4.3
)

cat(
  "Lateral real DDRTree saved. cells=", lateral_result$qc$cells,
  ", genes=", lateral_result$qc$genes,
  ", ordering_genes=", lateral_result$qc$ordering_genes,
  ", root_state=", lateral_result$qc$root_state,
  ", stage_cor=", lateral_result$qc$stage_pseudotime_cor,
  "\n",
  sep = ""
)
