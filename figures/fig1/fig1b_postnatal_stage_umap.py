"""Reproduce the developmental-stage inset used in Figure 1B."""
...
def load_data(h5ad_file: Path):
    ...
    if unexpected or sum(stage_counts.values()) != coords.shape[0]:
        raise ValueError(f"Stage annotations do not match the postnatal UMAP: unexpected={unexpected}")
...
def draw_figure(coords, stage_labels, stage_counts):
    ...
    for stage in draw_stages:
        ...
        ax.scatter(coords[indices, 0], coords[indices, 1], s=POINT_SIZE,
    ...
    ax.legend(handles=handles, loc="center left", bbox_to_anchor=(1.02, 0.50),
              ...
              handletextpad=0.45, labelspacing=0.90, borderaxespad=0)
...
def main() -> None:
    ...
    if args.check_inputs:
        print(f"Validated Figure 1B stage input: {coords.shape[0]:,} cells")
        return
    ...
    fig.savefig(output_dir / "fig1b_postnatal_stage_umap.png", dpi=DPI, **options)
    save_ai_editable_pdf(
        fig,
        output_dir / "fig1b_postnatal_stage_umap.pdf",
        bbox_inches=EXPORT_BBOX,
        title="Postnatal stage UMAP",
    )
    plt.close(fig)
